...
def load_css():
    ...
    <style>
        body, .stApp, .stChatFloatingInputContainer {
            background-color: #FFE0B2 !important; /* 옅은 하늘색으로 전체 배경색 변경 */
        }
        .stChatInputContainer {
            background-color: #FFE0B2 !important; /* 옅은 살구색 */
        }
        textarea {
...
def main():
    ...
    with st.sidebar:
        ...
        if st.session_state.show_examples:
            ...
            st.info("학생이 학교 밖에서 절도 및 도박 등의 사항으로 경찰서에서 보호처분을 받았습니다. 이 경우 학교에서 학생선도위원회를 열어 징계처분을 내릴 경우 이중 처벌이 되는 것은 아닌가요?")
            st.info("11월경에 학생선도위원회 개최가 예정되어 있습니다. 개최 사유는 해당 학생의 흡연 및 절도이며, 흡연은 9월, 절도는 10월에 각각 발생하였습니다. 학생선도위원회에서 흡연은 교내봉사, 절도는 특별교육이수로 조치를 나누어 해도 될까요?")

    # 스레드 ID 입력란을 자동으로 업데이트
    thread_id = st.session_state.thread_id

    st.title("중고등학교 학생선도규정 보조 챗봇")
